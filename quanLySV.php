    ...
    <div class="container">
        ...
        <div class="panel-heading">
            ...
            <div class="panel-body">
                  <button class="btn btn-success" onclick="window.open('inputSV.php', '_self')">Add</button>
...
foreach ($listLop as $value) {
    ...
    echo "<table class=\"table table-bordered table-hover\" id=\"$value\" style=\"display: none;\" style=\"width: 100%;\">
    <thead>
        <tr>
            <th>Mã sinh viên</th>
            <th>Họ tên</th>
    ...
    foreach($studentList as $std) {
        echo '<tr>
            ...
            <td>'.$std["sdt"].'</td>
            <td>'.$std["email"].'</td>
            <td><button class="btn btn-warning" onclick=\'window.open("inputSV.php?id='.$std['maSV'].'","_self")\'>Edit</button></td>
            <td><button class="btn btn-danger" onclick="deleteSV('.$std['id'].')">Delete</button></td>
        </tr>';
    }
    echo "</tbody>
    </table>";
...
?>
    ...
    <script type="text/javascript">
        function deleteSV(id) {
			option = confirm('Bạn có chắc muốn xoá sinh viên này không?')
			if(!option) {
				return;
			}
